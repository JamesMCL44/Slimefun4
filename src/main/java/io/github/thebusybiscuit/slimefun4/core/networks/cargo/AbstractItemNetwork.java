...
abstract class AbstractItemNetwork extends Network {
    ...
    private final ItemStack terminalPlaceholderItem = new CustomItem(Material.BARRIER, "&4沒有緩存項目");
    ...
    private void updateTerminal(Location l, BlockMenu terminal, int slot, int index, List<ItemStackAndInteger> items) {
        if (items.size() > index) {
            ...
            if (stack.getMaxStackSize() > 1) {
                ...
                lore.add(ChatColors.color("&7<左鍵: 請求一個 | 右鍵: 請求" + amount + "個>"));
            } else {
                lore.add(ChatColors.color("&7<左鍵: 請求一個>"));
            }
