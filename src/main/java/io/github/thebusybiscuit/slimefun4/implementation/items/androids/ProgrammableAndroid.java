...
public class ProgrammableAndroid extends SlimefunItem implements InventoryBlock, RecipeDisplayItem {
    ...
    public ProgrammableAndroid(Category category, int tier, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        new BlockMenuPreset(getId(), "機器人") {
            ...
            public void newInstance(BlockMenu menu, Block b) {
                menu.replaceExistingItem(15, new CustomItem(HeadTexture.SCRIPT_START.getAsItemStack(), "&a啟動/繼續運行"));
                ...
                menu.replaceExistingItem(17, new CustomItem(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&4暫停運行"));
                ...
                menu.replaceExistingItem(16, new CustomItem(HeadTexture.ENERGY_REGULATOR.getAsItemStack(), "&b内存核心", "", "&8\u21E8 &7單擊打開腳本編輯器"));
    ...
    public void openScript(Player p, Block b, String sourceCode) {
        ...
        menu.addItem(0, new CustomItem(Instruction.START.getItem(), SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions.START"), "", "&7\u21E8 &e左點擊 &7返回機器人控制台"));
        ...
        for (int i = 1; i < script.length; i++) {
            ...
            if (i == script.length - 1) {
                ...
                menu.addItem(slot, new CustomItem(Instruction.REPEAT.getItem(), SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions.REPEAT"), "", "&7\u21E8 &e左點擊 &7返回機器人控制台"));
                ...
                menu.addItem(i, new CustomItem(stack, SlimefunPlugin.getLocalization().getMessage(p, "android.scripts.instructions." + instruction.name()), "", "&7\u21E8 &e左點擊 &7編輯", "&7\u21E8 &e右點擊 &7刪除", "&7\u21E8 &eShift+右點擊 &7複製"));
                menu.addMenuClickHandler(i, (pl, slot, item, action) -> {
    ...
    protected void openScriptDownloader(Player p, Block b, int page) {
        ...
        menu.addItem(48, new CustomItem(HeadTexture.SCRIPT_UP.getAsItemStack(), "&e上傳腳本", "", "&6點擊 &7將你正在使用的腳本", "&7上傳到服務器"));
        ...
        menu.addItem(53, new CustomItem(HeadTexture.SCRIPT_LEFT.getAsItemStack(), "&6> 返回", "", "&7返回機器人控制台"));
    ...
    public void openScriptEditor(Player p, Block b) {
        ...
        menu.addItem(3, new CustomItem(HeadTexture.SCRIPT_NEW.getAsItemStack(), "&4> 創建新腳本", "", "&c刪除這腳本","&c並創建新腳本"));
        menu.addMenuClickHandler(3, (pl, slot, item, action) -> {
            openScript(pl, b, DEFAULT_SCRIPT);
            return false;
        });

        menu.addItem(5, new CustomItem(HeadTexture.SCRIPT_DOWN.getAsItemStack(), "&6> 下載腳本", "", "&e從伺服器中下載其他玩家上傳的腳本", "&e可以即下即用, 或者修改之後再使用"));
        menu.addMenuClickHandler(5, (pl, slot, item, action) -> {
            openScriptDownloader(pl, b, 1);
            return false;
        });

        menu.addItem(8, new CustomItem(HeadTexture.SCRIPT_LEFT.getAsItemStack(), "&6> 返回", "", "&7返回機器人控制台"));
    ...
    protected void editInstruction(Player p, Block b, String[] script, int index) {
        ...
        menu.setEmptySlotsClickable(false);
        menu.addItem(9, new CustomItem(HeadTexture.SCRIPT_PAUSE.getAsItemStack(), "&f不作任何動作"), (pl, slot, item, action) -> {
            String code = deleteInstruction(script, index);
            setScript(b.getLocation(), code);
            openScript(p, b, code);
            return false;
    ...
    public List<ItemStack> getDisplayRecipes() {
        ...
        for (MachineFuel fuel : fuelTypes) {
            ...
            lore.add(ChatColors.color("&8\u21E8 &7剩餘可用 " + NumberUtils.getTimeLeft(fuel.getTicks() / 2)));
            im.setLore(lore);
            item.setItemMeta(im);
            list.add(item);
        }
